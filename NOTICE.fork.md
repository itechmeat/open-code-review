...
| an empty `code_search` says why: file_patterns that match no file are named, a regex-looking literal (`a\|b`) is retried as a regex, a bare glob (`*.d.ts`) that finds nothing is retried under the reviewed tree's dependency directories, and an unscoped miss says dependencies were not searched | "No matches found" is no longer read as "does not exist" |

...
func TestMachineRunSummary(t *testing.T) {
	...
	got := machineRunSummary(m, 2, map[string]int64{"code_search": 3, "task_done": 1},
		&jsonLLMIdentity{Provider: "claude-code", Model: "opus"}, 54*time.Second, "caae40c0", 2043184, "merged 11→9 in 7s", 1, "fallback(per-file)")
	for _, want := range []string{"status=complete", "files=4/4", "comments=2", "tool_calls=4", "tokens=2043184", "llm_errors=1", `dedup="merged 11→9 in 7s"`, "grouping=fallback(per-file)",
	...
	got = machineRunSummary(nil, 0, nil, nil, time.Second, "", 0, "", 0, "")
	if !strings.Contains(got, "status=unknown") || !strings.Contains(got, "tool_calls=0") || !strings.Contains(got, "llm_errors=0") || strings.Contains(got, "dedup=") || strings.Contains(got, "grouping=") {

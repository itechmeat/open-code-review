...
### 一部のサブエージェントが失敗すると、実行は 3 で終了する

OCR はファイルごとの失敗を隔離し、1 つの問題のあるファイルが 20 ファイルのレビュー全体を
巻き込まないようにします。ほかのファイルの指摘はそのまま出力されます。ただしレビューは
不完全なので、実行は `0` ではなく `3` で終了し、stderr に再開用のセッションを出力します。
`ocr review --resume <session-id>` に同じ `--from`/`--to`/`--commit` を付けると、失敗した
ファイルだけをレビューします。完全に失敗した場合（成功したサブエージェントがゼロ）は `1` です。
`--max-tokens-budget` だけでスキップされたファイルでは `0` のままで、`--allow-partial` を
付けると部分的なレビューでも `0` になります。どのファイルが失敗したかは、JSON モードの
`manifest.coverage.failed` またはテキストモードの stderr を確認してください。

### CI での実行がローカルよりずっと遅い

よくある原因は 2 つです。

- **モデルのレート制限**——制限がかかると、LLM client はバックオフしてリトライします。
  `Retry-After` ヘッダーのない `429` は約 2・4・8・16・32 秒後にリトライされ（ジッター付き。
  サーバーの `Retry-After` が常に優先）、`429` の後は同時に送る LLM リクエスト数を一時的に
  下げ、成功に応じて戻します（`[ocr] Provider rate limit hit; lowering concurrent LLM requests to N`）。
  最初から制限に触れないよう、`--concurrency` を下げてください（`4` など）。

...
| `--allow-partial` | — | `false` | Завершаться с кодом `0` вместо `3`, когда результаты ревью выведены, но часть выбранных файлов обработать не удалось. Для случаев, где частичное ревью не должно считаться ошибкой. |
...
| `--include <patterns>` | — | — | Шаблоны в стиле gitignore (через запятую, например `**/*.bat`) для файлов, которые нужно проверить, даже если их расширение или путь отфильтрованы по умолчанию; объединяются с include из `rule.json`. `--exclude` по-прежнему главнее. |
| `--include-docs` | — | `false` | Проверять также файлы документации (`**/*.{md,mdx,markdown,rst,adoc}`) встроенным правилом `documentation.md`: оно сверяет с кодом записи CHANGELOG, описанные флаги, пути и примеры, а не стиль текста. По умолчанию выключено. |
...
| `3` | Частичное ревью — результаты (находки, `session_id`, покрытие) выведены, но часть выбранных файлов обработать не удалось, например из-за ограничений частоты запросов или тайм-аутов. В stderr указана сессия; проверьте оставшиеся файлы командой `ocr review --resume <session-id>` с теми же `--from`/`--to`/`--commit`. Файлы, пропущенные только из-за `--max-tokens-budget`, не учитываются, а `--allow-partial` превращает этот код в `0`. |
